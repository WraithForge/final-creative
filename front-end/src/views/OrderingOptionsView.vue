<template>
  <div class="ordering">
    ...
    <br>
    <div class='contact'>
      <p>Type in item ideas here to save them for later!</p>
      <input v-model="username" placeholder="Your Name">
      <input v-model="itemname" placeholder="Item Name">
      <input v-model="itemdesc" placeholder="Description">
      <br>
      <button @click="upload">Upload</button>
    </div>
    <br>
    <div class='contact'>
      <p>All of the saved items will appear here!</p>
      <ul>
        <li v-for='item in items' :key="item.username">
          <b>{{item.name}}</b>, {{item.description}} - <i>{{item.creator}}</i>
        </li>
      </ul>
    </div>
  </div>
  ...
  export default {
    ...
    methods: {
      ...
      async deleteItem(item) {
        try {
          await axios.delete("http://localhost:3000/api/items" + item.name);
          this.item = null;
          this.getItems();
          return true;
        } catch (error) {
      ...
      async upload(){ //this will need to be some objects made in the end of each generated thing?
        //console.log("yes?");
        try {
          let postItem = await axios.post('http://localhost:3000/api/items', {
            name: this.itemname,
            description: this.itemdesc,
            creator: this.username,
          });
          this.addItem = postItem.data;
        } catch (error) {
          //console.log(error);
        }
